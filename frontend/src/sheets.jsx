...
function WeightInput({ value, setValue, unit }) {
  ...
  return <>
    <div className="bwstep">
      <button className="bw-pm" onClick={() => onSlide(value - 0.1)} aria-label={t('Decrease by 0.1')}><Icon name="minus" /></button>
      <div className="bw-read">{fmtNum(value)}<span className="u"> {unit}</span></div>
      <button className="bw-pm" onClick={() => onSlide(value + 0.1)} aria-label={t('Increase by 0.1')}><Icon name="plus" /></button>
    </div>
...
function ExercisePicker({ onPick, close, routineId, quickAdd }) {
  ...
  let base = all.filter(e =>
    ...
    (!ql || exerciseSearchText(e).includes(ql)))
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'fitproplayer-plan-' + todayISO() + '.json'
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own Fit Pro Player — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
