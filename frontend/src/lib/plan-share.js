...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR Fit Pro Player — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { fitproplayer_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.fitproplayer_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an Fit Pro Player plan file'))
  }
