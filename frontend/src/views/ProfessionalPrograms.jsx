...
export default function ProfessionalPrograms() {
  ...
  return <div className="narrow professional-page"><AppHeader title="Programas" subtitle="Prescrições e versões reutilizáveis" backTo="/professional" /><ProfessionalWorkspaceNav />{error && <p role="alert" className="error">{error}</p>}{message && <p role="status">{message}</p>}{busy ? <p role="status">Carregando programas…</p> : editor ? <ProfessionalProgramEditor exercises={EXDB} initialPlan={editor.initialPlan} onCancel={() => setEditor(null)} onPublish={plan => publish(editor.program.id, plan)} /> : <>
