...
export default function InviteLanding() {
  ...
  useEffect(() => { if (!auth.user?.id) return; withTimeout(repo.previewInvite(code), 10000).then(rows => { setPreview(rows?.[0] || null); if (!rows?.length) setError('Convite inválido ou expirado.') }).catch(cause => setError(cause?.message === 'request-timeout' ? 'A consulta do convite demorou mais que o esperado.' : 'Não foi possível consultar este convite.')).finally(() => setBusy(false)) }, [auth.user?.id, code])
  const accept = async () => { setError(''); try { await repo.acceptInvite(code); sessionStorage.removeItem('fpp-pending-invite'); setMessage('Vínculo criado.'); navigate('/student/professionals?invite=accepted', { replace: true }) } catch { setError('Não foi possível aceitar este convite. Ele pode ter expirado ou já ter sido utilizado.') } }
