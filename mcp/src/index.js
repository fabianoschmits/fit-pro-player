#!/usr/bin/env node
/* Fit Pro Player MCP server — stdio transport. The LLM client (Claude Desktop, Cursor, …) spawns
...
// Fail fast on bad config so a misnamed FITPROPLAYER_DATA doesn't silently answer every call with
...
try {
  init()
  const u = getUser()
  console.error(`[fitproplayer-mcp] serving profile ${u.name} (${u.id})`)
} catch (e) {
  console.error(`[fitproplayer-mcp] ${e.message}`)
