...
# Fit Pro Player does not redistribute or relicense them. See NOTICE.md.
set -euo pipefail
cd "$(dirname "$0")/.."
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
↓ Downloading exercise media (~140 MB) from github.com/hasaneyldrm/exercises-dataset
  ...
  Used under that dataset's terms, not Fit Pro Player's AGPL; Fit Pro Player does not redistribute them.
